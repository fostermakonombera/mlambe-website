...
include("conn.php");
$sql="SELECT pic,title,s_description FROM services";
    ...
    <nav id="nav" class="navbar navbar-expand-lg navbar-light">
            ...
            <a class="navbar-brand" href="#">
              ...
              <div class="collapse navbar-collapse justify-content-end align-center" id="navMain">
                <ul class="navbar-nav">
                   <li class="nav-item">
                    <a class="nav-link active bg-success rounded" aria-current="page" href="#">Services</a>
                  </li>
          ...
          <h2 class="bg-success text-white rounded p-4">OUR SERVICES<h2>
          </div>
         ...
         if($result->num_rows>0){
          while($row=$result->fetch_assoc()){
            echo"<div class='card col-sm-12 col-md-6 col-lg-5 shadow text-center bg-white text-black  m-auto mb-3 py-4'><!--<img src='".$row['pic']."' alt='Department picture'>-->
            <div class=' img-thumbnail border shadow'>
               <img  class='card-img-top'  src='photos/" . htmlspecialchars($row['pic']) . "'
                   alt='" . htmlspecialchars($row['title']) . "'>
            </div>
            <h2 class='text-success'>".$row['title']."</h2>
            <p>".$row['s_description']."</p></div><br>";
          }
        }
          else{
